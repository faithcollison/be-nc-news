...
exports.selectArticles = (
  ...
  let total_count;
  if (topicQuery) {
    total_count = db.query(`SELECT COUNT(*) FROM articles WHERE topic = $1;`, [
      topicQuery,
    ]);
  } else {
  ...
  return Promise.all([total_count, articleQuery]).then(
    ([totalCount, articleResponse]) => {
      return {
        totalCount: totalCount.rows,
        queryResponse: articleResponse.rows,
      };
    }
  );
};
